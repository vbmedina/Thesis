"""
Data Split Graphic (Seaborn + Reds palette)
- 5 splits (K=1..5)
- Each split shows 5 equal blocks (each 20% of total data)
- The designated test block shifts from block 5 down to 1 across K=1..5
- Test uses seaborn 'Reds' (deep red); training blocks are white
- Validation: 10% of the TRAINING set (subset of train; not shown in the heatmap)
This script saves a high-res PNG.
"""
...
# For K = 1..5, test block indices (1-indexed) are [5,4,3,2,1]
TEST_BLOCKS = [5, 4, 3, 2, 1]

# --- Build a binary matrix where rows=K, cols=blocks ---
# 0 = Train (white), 1 = Test (red)
...
sns.heatmap(
    ...
    cbar=False,
    linewidths=2,
    linecolor="#d1d5db",  # visible grid so white cells are distinct
    ax=ax,
    vmin=0,
    vmax=1,
)

# Subtle off-white background so white cells stand out
