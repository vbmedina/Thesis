...
def show(name, *frames):
    ...
    print(f"\n[{name}]")
    hdr = "  set      rows   ≤5   5-6   6-7   ≥7"
    print(hdr, "\n  " + "-"*(len(hdr)-2))
    for tag, df_, ct in zip(tags, frames, c):
        print(f"  {tag:<10}{len(df_):7,} {ct['≤5']:5} {ct['5-6']:5} "
              f"{ct['6-7']:5} {ct['≥7']:5}")
...
if __name__ == "__main__":
    ...
    print(f"Retained {len(big)} strains "
          f"({sz[big].sum():,} / {len(df):,} rows)")
    ...
    for strain in big:
        ...
        val_idx = rng.choice(train_idx,
                             size=int(0.10*len(train_idx)),
                             replace=False)
        ...
        trn_df.to_csv(out/"train.csv", index=False)
        val_df.to_csv(out/"val.csv",   index=False)
        tst_df.to_csv(out/"test.csv",  index=False)
    ...
    # Cold-regime split 
    cold_cmpd = rng.choice(df["Molecule_ChEMBL_ID"].unique(),
                           size=int(0.10*df["Molecule_ChEMBL_ID"].nunique()),
                           replace=False)
    cold_str  = rng.choice(df["strain_norm"].unique(),
                           size=int(0.10*df["strain_norm"].nunique()),
                           replace=False)
    ...
    show("cold_regime", warm, cold_cmpd_, cold_str_, cold_both)
    print("\n✓ cold_regime CSVs - ", out)
